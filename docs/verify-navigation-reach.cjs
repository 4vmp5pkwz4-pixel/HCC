...
/* the registry, read from the document rather than from the module, so the check cannot
   ...
   ── AND A LITERAL IS NOT THE REGISTRY ANY MORE ────────────────────────────────
   This read S3_VIEW_NAMES with a regex over the source and counted the keys, which
   worked for as long as every key was written out by hand. It stopped the moment ten
   registries were folded into one declaration table: the last two laboratories reach
   S3_VIEW_NAMES through a spread, so the literal holds 88 names for a router that knows
   90, and this file reported the wrist picker offering MORE than exist.

   The regex is kept for the literal half and joined by the declarations, parsed the same
   way from the same file. Reading api/manifest.json instead would have been shorter and
   would have made this check depend on an artifact regenerated by the pipeline it is
   supposed to be independent of — the whole point of parsing the document is that a
   headset menu can be checked by someone holding only the document. */
const REG = (() => {
  const m = html.match(/const S3_VIEW_NAMES=\{([\s\S]*?)\};/);
  const literal = [...m[1].matchAll(/([a-zA-Z0-9_]+):'/g)].map(x => x[1]);
  const d = html.match(/const LAB_DECLARATIONS=Object\.freeze\(\[([\s\S]*?)\n\]\);/);
  const declared = d ? [...d[1].matchAll(/\{id:'([a-zA-Z0-9_]+)'/g)].map(x => x[1]) : [];
  return [...new Set([...literal, ...declared])];
})();
