#!/usr/bin/env node
'use strict';
/* ── A CHECK THAT WAS LEFT ON A BRANCH ───────────────────────────────────────
   This arrived as docs/reconcile-ancient-phase-b-red.cjs on
   reconcile/4.150-ancient-chronometry, a branch that was never merged and whose
   index.html is seven releases behind. Run against main it passed five of its
   six clauses and failed one — the live contract exposed only ONE of the
   classifier's seven verdicts, so a reader could see the rule refuse and never
   see it accept. The laboratory now shows the whole decision surface, computed
   at render time, and this is kept as a standing check rather than a one-off
   reconciliation script: the name says verify, and the suite picks it up. */
const fs=require('fs');
let fail=0;
