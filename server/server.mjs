...
export const TOOLS = [
  ...
  { name: 'list_connections', description: 'How laboratories connect, in the four forms this atlas has. TYPED: hand-written edges across the laboratory catalogue, each with the KIND of relationship (exact, invariant, representation, coupling, limit, causal, analogy, contrast), the claim somebody wrote, and its epistemic status — served with a derived evidence_tier (theorem, measured, model, analogy, conditional, context, literature, open) so "only theorem-grade edges" is answerable without parsing seventy-one free-text statuses. ROUTE: the quantity bus carries a number along it. REFUSAL: the bus found the coupling admissible and DECLINED it, carrying the sentence saying why — the boundary an agent can plan around. SOURCED: a cited connection with its URLs and its claim kind. FAMILY: a QUESTION several laboratories answer in different units — an edge says two laboratories are related, a family says several are answering the same question and disagree about what the answer even is; each row names the bus key carrying its number, and the family declares whether it is one number or a relation. Filter by laboratory, kind or tier; an unknown value is refused with the known ones named.',
    inputSchema: { type: 'object', properties: { lab: { type: 'string' },
      kind: { type: 'string', enum: ['route', 'refusal', 'sourced', 'typed', 'family'] },
  ...
  { name: 'list_measurements', description: 'What the atlas has measured about its own behaviour, from four artifacts generated by walking it headlessly. SENSITIVITY: 484 inputs driven one at a time — 407 respond and 25 are DEAD, meaning moving them changes no output at all, which no contract reveals and which decides whether a sweep measures anything. TRANSFERS: the exponent between an input and an output per route. REACH: 511 composed chains, each judged a power law, not a power law, or unjudged. LIVENESS: every view and whether it visibly responds to its own controls. Each artifact reports its own release stamp and whether it was measured on THIS release; dated artifacts are still served because a stale measurement read as a fresh one is worse than none.',
    inputSchema: { type: 'object', properties: {
      kind: { type: 'string', enum: ['sensitivity', 'transfers', 'reach', 'liveness'] },
      lab: { type: 'string' } }, additionalProperties: false },
    call: a => CORE.measurements({ kind: a.kind || null, lab: a.lab || null }) },
...
export const server = createServer(async (req, res) => {
  ...
  try {
    ...
    /* WHAT THE ATLAS HAS MEASURED ABOUT ITSELF. Four artifacts generated by walking
       it headlessly — which inputs are DEAD, what exponent connects two quantities,
       what a control reaches through another laboratory, and whether a view responds
       at all — and until now none of them was reachable by anything but reading a
       file out of the repository. Each reports its own release stamp, because one of
       them is deliberately dated and a stale measurement read as a fresh one is
       worse than no measurement. */
    if (p === '/api/v1/measurements') {
      const q = new URL(req.url, 'http://x').searchParams;
      try { return json(res, 200, CORE.measurements({ kind: q.get('kind'), lab: q.get('lab') })); }
      catch (e) { return json(res, httpCodeFor(e), errBody(e)); }
    }
    m = p.match(/^\/api\/v1\/measurements\/([^/]+)$/);
    if (m) { try { return json(res, 200, CORE.measurements({ kind: decodeURIComponent(m[1]) })); }
      catch (e) { return json(res, httpCodeFor(e), errBody(e)); } }
