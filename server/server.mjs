...
export const TOOLS = [
  ...
  { name: 'list_measurements', description: 'What the atlas has measured about its own behaviour, from four artifacts generated by walking it headlessly. SENSITIVITY reports its own measured input, response and dead-control counts. TRANSFERS reports the measured exponent between an input and an output per route. REACH reports its own measured composed-chain count and classifies each chain as a power law, not a power law, or unjudged. LIVENESS reports every measured view and whether it visibly responds to its own controls. Each artifact reports its own release stamp and whether it was measured on THIS release; dated artifacts are still served because a stale measurement read as a fresh one is worse than none.',
