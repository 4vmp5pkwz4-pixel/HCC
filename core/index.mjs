...
export const NAMED_OPEN_PROBLEMS = Object.freeze([
  ...
  ['atlas.boot_suite_cost', 'the self-test suite runs unconditionally on every page load and costs about ten seconds of a twenty-six second boot, measurable through HCC_API.selftest.cost() and attributable through HCC_API.selftest.slowest(); two clauses account for a third of it, and nothing decides which checks a reader should pay for on arrival and which belong to CI against a published artifact — so the cost grows whenever anyone adds a clause, which is how it reached ten seconds without being noticed'],
  ['atlas.numerical_controls', 'nothing in an instrument declaration distinguishes a PHYSICAL input from a NUMERICAL one — an integration step, a sample count, a tolerance — so api/reach.json cannot tell a chain that carries a dependence from one that carries a discretisation error, and lists ns.step alongside a black-hole mass; a solver control that moves a published output across two laboratories is a statement about convergence and is not the same statement as a physical influence'],
