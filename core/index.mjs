...
export const NAMED_OPEN_PROBLEMS = Object.freeze([
  ...
  ['atlas.one_input_many_authorities', 'a quasar\'s Eddington limit, a supernova\'s light curve and an accretion disk\'s output are all luminosities in erg/s and all three are admissible into the photometer, but one input has exactly one driver — so two are refused on structure and the quasar holds the input only because it was declared first. The one-driver rule is right: two laboratories declaring the same fact cannot both be trusted about it. What is missing is any way for the bus to express \'evaluate this instrument once per source\', so an instrument several sources could legitimately feed gets one by accident of declaration order, and a reader has no way to see that the others were equally good. The refusals say so; nothing measures how often it happens'],
  ...
  ['atlas.near_leg_fits_are_mostly_poor', 'publishing the near leg\'s R² revealed that only 8 of the 44 chains in api/reach.json are power laws across the ranges they were fitted on. Thirty-four fail on the NEAR leg, whose fit quality had never been written down: ns.central_density → ns.mass fits at R² 0.9089 because the TOV mass has a maximum-mass turnover, and three others sit near 0.72. Those are not errors — they are laboratories whose own response is not a power law — but it means the reach graph is mostly a graph of local derivatives that do not extend, and nothing yet measures over what SUB-range each one does hold. The verdict says a chain is not a scaling law; it does not yet say where it becomes one'],
