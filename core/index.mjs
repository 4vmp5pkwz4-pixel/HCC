...
export const NAMED_OPEN_PROBLEMS = Object.freeze([
  ...
  ['atlas.a_laboratory_is_registered_in_fourteen_places', 'adding the eighty-ninth laboratory required entries in fourteen separate registries — view names, three-language names, three-language descriptions, a purpose line, a category, a premium visual domain, a nexus cluster, a prediction class, a prediction target, a catalogue list, a scene group, a visibility switch, a setup dispatch and an update call. Nothing derives one from another and nothing enumerates them, so the only way to find the list is to add a laboratory and let five self-test clauses and two page-blanking crashes tell you what you missed. The clauses caught every omission, which is the system working; that the list has fourteen entries rather than one is the defect. A laboratory should be declared once'],
