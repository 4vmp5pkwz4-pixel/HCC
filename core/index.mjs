...
export const NAMED_OPEN_PROBLEMS = Object.freeze([
  ...
  ['atlas.the_name_trap_caught_the_next_laboratory_it_was_written_about', 'the open problem below was filed when the predictability horizon declared inputs called `steps` and `tolerance` and the quantity bus immediately proposed three meaningless couplings. It closed with the words: nothing stops the next laboratory from calling an input `n` or `count` or `eps` and drawing the same nonsense. The next laboratory was the clock exchange, written in the same session by the same hand with that paragraph already on the page, and it declared an input called `neighbours` -- whereupon the bus proposed a coupling from the 600-cell, whose vertices have neighbours too. One laboratory, and the prediction came true. Renaming it to `fit_window` cleared the proposal, which is the same non-fix as last time: a habit each author has to remember rather than a property of the bus. The bus could refuse a coordinate name that appears in more than one laboratory with unrelated meanings, or require every non-dimensionless input to declare what it is a coordinate OF, and it does neither. Writing an open problem about a trap is not the same as not falling into it'],
  ['atlas.a_generic_name_and_no_unit_is_not_a_coordinate', 'the quantity bus admits a coupling when the unit string agrees AND the coordinate name agrees, and the second condition is what stops it wiring a temperature into a temperature that has nothing to do with it. The predictability horizon exposed the hole in that rule by accident: it declared an input called `steps` and one called `tolerance`, both dimensionless, and the bus immediately proposed three admissible couplings — a wave solver`s step count, a quantum propagator`s step count and a holonomy transport`s tolerance — every one of them meaningless. The name agreed because the name was generic and the unit agreed because there was no unit. Two conditions that are each vacuous do not become a check by being taken together. The inputs were renamed and the proposals vanished, which fixes this laboratory and not the rule: nothing stops the next laboratory from calling an input `n` or `count` or `eps` and drawing the same nonsense, and the atlas has no notion of a name being too generic to be a coordinate'],
