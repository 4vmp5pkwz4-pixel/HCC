...
export const NAMED_OPEN_PROBLEMS = Object.freeze([
  ...
  ['atlas.one_input_many_authorities', 'eight inputs in this atlas have more than one admissible source and nine sources lost; the photometer\'s luminosity is wanted by a quasar, a supernova and an accretion disk, and the quasar holds it because it was declared first. That is now COUNTED — HCC_API.bus.contested() and api/manifest.json publish which inputs were contested, what was chosen and what the alternatives were, and a clause holds every loser to a refusal in writing. What is still missing is the counterfactual: the bus cannot express \'evaluate this instrument once per source\', so the atlas can say a supernova would have been an equally admissible authority and cannot say what the chain would have measured if it had been'],
