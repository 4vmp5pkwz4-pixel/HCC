...
export const NAMED_OPEN_PROBLEMS = Object.freeze([
  ...
  ['atlas.passport_recomputes_a_double_integral', 'building the passport for a selected object in the contact/action laboratory calls actDiagnostics(), whose Reeb station evaluates a 512x512 Gauss double integral; the three calls inside one row-build were reduced to one, but a CPU profile of boot still attributes about 1.5 s of self time to pspGauss, because the clause that walks every selectable object pays it once per object. The passport is the reader\'s click path, not a test: the right shape is a cache keyed on the resolved (station, a, b) that the laboratory invalidates when it moves, and that is not written because the returned object is handed to the renderer and it is not yet established that nothing mutates it'],
