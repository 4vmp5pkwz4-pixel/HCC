...
NEW_BUILD='self-description-has-one-authority-2026.09.17.15'
...
def sub_once(path, pattern, repl, flags=0):
    ...
    if n != 1: raise SystemExit(f'{path}: expected one match for {pattern!r}, got {n}')
    write(path,s2)
def replace_once(path, old, new):
    s=text(path); n=s.count(old)
    if n != 1: raise SystemExit(f'{path}: expected one exact occurrence, got {n}: {old[:120]!r}')
    write(path,s.replace(old,new,1))

manifest=json.loads(text('api/manifest.json'))
counts=manifest['counts']
labs=int(counts['laboratories']); worlds=int(counts['worlds']); instruments=int(counts['instruments'])
...
sub_once('README.md',
    r'The live laboratory registry is authoritative: \*\*\d+ laboratories across \d+ worlds\*\* at the\s+v\d+\.\d+\.\d+ release boundary\. The generated `api/manifest\.json` is measured by walking the\s+application headlessly; prose counts are descriptive, never an independent source of truth\.',
    'The live census is `api/manifest.json` (`counts.laboratories`, `counts.instruments`,\n`counts.worlds`) at the release declared by `version.json`. The manifest is measured by walking\nthe application headlessly; prose counts are descriptive, never an independent source of truth.', re.S)
...
sub_once('server/server.mjs',r'Each artifact reports its own release stamp and whether it was measured on THIS release — one of them is deliberately dated, and a stale measurement read as a fresh one is worse than none\.',
...
if v.get('version') != OLD_VERSION or v.get('build') != OLD_BUILD: raise SystemExit(f'unexpected version.json base: {v}')
...
idx=text('index.html')
idx,nv=re.subn(r"(HCC_VERSION\s*=\s*['\"])"+re.escape(OLD_VERSION)+r"(['\"])",r'\g<1>'+NEW_VERSION+r'\2',idx,count=1)
idx,nb=re.subn(r"(HCC_BUILD\s*=\s*['\"])"+re.escape(OLD_BUILD)+r"(['\"])",r'\g<1>'+NEW_BUILD+r'\2',idx,count=1)
if nv != 1 or nb != 1: raise SystemExit(f'index identity replacement failed: version={nv} build={nb}')
write('index.html',idx)

manifest=json.loads(text('api/manifest.json'))
if manifest.get('version') != OLD_VERSION or manifest.get('build') != OLD_BUILD:
    raise SystemExit(f'unexpected manifest identity: {manifest.get("version")}/{manifest.get("build")}')
...
let pass=0,fail=0; const ok=(n,c,d='')=>{if(c){pass++;console.log(`  PASS — ${n}${d?` :: ${d}`:''}`)}else{fail++;console.log(`  FAIL — ${n}${d?` :: ${d}`:''}`)}};
...
const lm=llms.match(/\b(\d+)\s+laboratories\b/); ok('llms.txt carries no stale laboratory authority',!lm||Number(lm[1])===manifest.counts.laboratories,lm?lm[1]:'structural wording');
ok('MCP prose carries no frozen laboratory count',!/over\s+\d+\s+laboratories/i.test(server));
const payload=CORE.openProblems(); const probs=payload.problems||payload.open_problems||[]; const find=id=>probs.find(p=>p.id===id)||{};
