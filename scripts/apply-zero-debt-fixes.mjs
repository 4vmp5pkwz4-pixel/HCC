...
once("atlasNav:{scope:'global'}, navPanel:{scope:'global'}, atlasPanel:{scope:'global'}, xrCheck:{scope:'global'},","atlasNav:{scope:'global'}, navPanel:{scope:'global'}, atlasPanel:{scope:'global'}, predictivePanel:{scope:'global'}, xrCheck:{scope:'global'},",'predictive panel scope');
once('  <button class="btn" data-panel="atlasPanel" title="Atlas QA and runtime manifest · Q">QA Atlas</button>','  <button class="btn" data-panel="atlasPanel" title="Atlas QA and runtime manifest · Q">QA Atlas</button>\n  <button class="btn" data-panel="predictivePanel" title="Predictive foundation">Predictive foundation</button>','predictive panel recall control');
...
once('  const STACK_N=32;','  const STACK_N=HCC_SI_KINDS.length;','scale-axis capacity follows quantity registry');

const start=s.indexOf("function nexusStructuralCalibration(kind='all'){");const end=s.indexOf('function nexusEnsureHypothesisLayer(){',start);if(start<0||end<0)throw new Error('nexusStructuralCalibration boundaries not found');
const newFn=`function nexusStructuralCalibration(kind='all'){
  ...
  for(const e of pairs.values()){
    ...
    for(const q of NEXUS_RELATIONS){if(kind!=='all'&&q.type!==kind)continue;if((q.a===e.a&&q.b===e.b)||(q.a===e.b&&q.b===e.a))continue;adj.get(q.a)?.add(q.b);adj.get(q.b)?.add(q.a);}
