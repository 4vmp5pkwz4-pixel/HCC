#!/usr/bin/env python3
"""
THE ZODIAC IN THREE DIMENSIONS — the catalogue the atlas embeds, and how it was made.
...
OUT  = sys.argv[2] if len(sys.argv) > 2 else 'data/zodiac-3d.json'
...
sel = [r for r in T if r['iau'] in ECL]

# ── 3. the figures ─────────────────────────────────────────────────────────────────
fig = {c['iau']: c['lines'] for c in man['constellations'] if c.get('iau') in ECL}
figHip = {h for L in fig.values() for poly in L for h in poly}
missing = figHip - {r['hipId'] for r in sel}
assert not missing, f'figure stars outside the selection: {missing}'
...
for r in sel:
    ...
    out.append(dict(
        ...
        con=r['iau'], zod=r['iau'] in ZOD, name=proper, bayer=bayer, flam=flam,
...
meta = dict(generated='scripts/build-zodiac-3d.py', rule=dict(brightV=BRIGHT_V, hipFloorMas=HIP_FLOOR_MAS),
