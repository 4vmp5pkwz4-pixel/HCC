    ...
    '/api/v1/connections': { get: {
      ...
      parameters: [
        ...
        { name: 'kind', in: 'query', required: false,
          schema: { type: 'string', enum: ['typed', 'route', 'refusal', 'sourced', 'family'] },
          description: 'typed — a hand-written edge with its relationship kind, claim and epistemic status; route — a number travels the edge; refusal — admissible and declined, carrying the reason; sourced — a cited connection with its URLs and its claim kind; family — a question several laboratories answer in different units, each row naming the bus key that carries its number' },
        { name: 'tier', in: 'query', required: false,
          schema: { type: 'string', enum: ['theorem', 'measured', 'open', 'analogy', 'conditional', 'model', 'literature', 'context', 'unclassified'] },
          description: 'evidence tier, derived from each typed edge\'s own epistemic status by a declared ordered rule. Only typed edges carry one; asking for a tier on a kind that has none is answered with a note saying so, not an empty list.' }],
      responses: { 200: { description: 'ok' },
        422: { description: 'an unknown kind, refused with the known kinds named' } } } },
    '/api/v1/measurements': { get: {
      summary: 'what the atlas has measured about its own behaviour: which inputs are dead, the exponent between two quantities, what a control reaches through another laboratory, and whether a view responds at all',
      parameters: [
        { name: 'kind', in: 'query', required: false,
          schema: { type: 'string', enum: ['sensitivity', 'transfers', 'reach', 'liveness'] },
          description: 'one artifact instead of all four; an unknown kind is refused with the known ones named' },
        { name: 'lab', in: 'query', required: false, schema: { type: 'string' },
          description: 'only rows mentioning this laboratory. A name no measurement mentions is answered with a note saying so.' }],
      responses: { 200: { description: 'ok — every artifact reports its own release stamp and whether it was measured on this release' },
        422: { description: 'an unknown kind, refused with the known kinds named' } } } },
    '/api/v1/measurements/{kind}': { get: { summary: 'one measurement artifact',
      parameters: [{ name: 'kind', in: 'path', required: true,
        schema: { type: 'string', enum: ['sensitivity', 'transfers', 'reach', 'liveness'] } }],
      responses: { 200: { description: 'ok' }, 422: { description: 'an unknown kind' } } } },
